...
DISCA_fwd <- function(X,Y,e=10^-3,
  ...
  U_n = matrix(0,p,0)  # each column represents a direction.
  ...
  for (DIM in 1:(p-1)){  # Check if the reduced dimension can reach DIM.
    ...
    u_ult = (1/sqrt(sum((U%*%u)^2)))*U%*%u    # New direction in U_n.
  ...
  V_n = matrix(0,q,0)  # each column represents a direction.
  ...
  if (DIM==q-1){
    test_sum <- dcovU.test(Y_0%*%V, X_0, R = R, alpha = alpha)
    dc_0 = test_sum$estimate
    threshold = test_sum$threshold
    DC_Y = c(DC_Y,dc_0)
    THRES_Y = c(THRES_Y,threshold)
