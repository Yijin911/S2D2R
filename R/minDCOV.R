#' Minimize the distance covariance of \eqn{Xu} and \eqn{Y} under certain constraint.
#'
#' @export
#' @importFrom stats runif
#' @description \code{minDCOV} is used to solve the non-convex optimization problem of
#' minimizing the distance covariance of \eqn{Xu} and \eqn{Y} under the constraint that
...
minDCOV <- function(Xd,gM,u0=c(1,rep(0,ncol(Xd)-1)),e=10^-3,
  ...
  M_plus <- Mplus(gM,Xd)  # The M plus matrix.
  M_minus <- Mminus(gM,Xd)  # The M minus matrix.
